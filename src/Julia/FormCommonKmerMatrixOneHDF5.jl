...
#Example usage
#julia -p 2 FormCommonKmerMatrix.jl --counts_file /raid2/labs/Koslicki_lab/koslickd/CommonKmers/TrainingOnRepoPhlAn/Counts/Counts20C/ --size_of_chunk 2 --file_names testFileNames.txt  -o test_out.h5 -k 20
...
function parse_commandline()
    ...
    @add_arg_table s begin
        "--counts_file", "-c"
            help = "File name of the HDF5 file containing all the kcounts"
        "--size_of_chunk", "-s"
...
function main()
	...
	if parsed_args["counts_file"]==nothing
	    error("Must provide the counts HDF5 file, use --help to see usage")
	end
	...
	@everywhere counts_file = parsed_args["counts_file"]
		...
		for i = 1:chunk_size:num_seqs
			...
			if i>=j
				...
				@parallel for ii = i:i+chunk_size-1
					if ii<=num_seqs
						iKmers = HDF5.h5read(counts_file,"/$(file_names[ii])");
						for jj=j:j+chunk_size-1
							if jj<=num_seqs
								jKmers = HDF5.h5read(counts_file,"/$(file_names[jj])");
